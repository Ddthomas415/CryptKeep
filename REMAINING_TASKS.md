...
9. Make the strategy registry fail closed before new discovery wiring lands.
   ...
   Implementation proof is ready for independent review: the registry returns
   `ok=false`, `action=hold`, and `reason=unknown_strategy` for explicit
   unknown names while preserving the existing missing-name `ema_cross`
   default.
