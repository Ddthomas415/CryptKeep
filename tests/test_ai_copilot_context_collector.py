...
def test_collect_incident_context_includes_runtime_logs_and_alerts(tmp_path, monkeypatch):
    ...
    (runtime / "flags" / "system_guard.json").write_text(json.dumps({"state": "RUNNING"}), encoding="utf-8")
    (runtime / "flags" / "kill_switch.json").write_text(json.dumps({"armed": False}), encoding="utf-8")
