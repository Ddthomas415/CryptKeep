...
def test_recent_first_lines_do_not_contain_mixed_double_comparison_tokens() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "!=" not in body, (
            f"Phase {phase} first checklist line should not contain not-equal tokens"
        )
        assert "<=" not in body, (
            f"Phase {phase} first checklist line should not contain less-equal tokens"
        )
        assert ">=" not in body, (
            f"Phase {phase} first checklist line should not contain greater-equal tokens"
        )
