...
def test_recent_first_lines_do_not_contain_double_quote_seventh() -> None:
    for phase, body in _recent_first_line_bodies():
        assert '""' not in body, (
            f"Phase {phase} first checklist line should not contain doubled double-quotes"
        )
        assert "''" not in body, (
            f"Phase {phase} first checklist line should not contain doubled single-quotes"
        )
