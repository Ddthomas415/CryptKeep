...
def test_recent_first_lines_have_single_purpose_delimiter() -> None:
    for phase, body in _recent_first_line_bodies():
        assert body.count(" to ") == 1, (
            f"Phase {phase} first checklist line should include exactly one purpose delimiter ` to `"
        )
