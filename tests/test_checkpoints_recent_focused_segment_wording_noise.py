...
def _recent_focused_segments() -> list[tuple[int, str]]:
    ...
    for line in lines:
        ...
        if len(segs) != 3:
            continue
        rows.append((phase, segs[0]))

    assert rows, "No phase verification lines found in CHECKPOINTS.md"
    return rows[-RECENT_WINDOW_SIZE:]


def test_recent_focused_segment_has_single_focused_token() -> None:
    for phase, seg in _recent_focused_segments():
        assert seg.count("focused") >= 1, (
            f"Phase {phase} focused segment should contain at least one 'focused' token"
        )


def test_recent_focused_segment_has_single_whole_word_pass_token() -> None:
    for phase, seg in _recent_focused_segments():
        pass_words = re.findall(r"\bpass\b", seg)
        assert len(pass_words) == 1, (
            f"Phase {phase} focused segment should contain exactly one whole-word 'pass' token"
        )
