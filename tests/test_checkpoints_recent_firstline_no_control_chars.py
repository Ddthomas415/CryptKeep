...
def test_recent_first_lines_do_not_contain_control_characters() -> None:
    for phase, body in _recent_first_line_bodies():
        assert not any(ch in FORBIDDEN_CONTROL_CHARS for ch in body), (
            f"Phase {phase} first checklist line should not contain control characters"
        )
