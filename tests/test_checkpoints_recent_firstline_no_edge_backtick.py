...
def test_recent_first_lines_do_not_start_or_end_with_backtick() -> None:
    for phase, body in _recent_first_line_bodies():
        assert not body.startswith("`"), (
            f"Phase {phase} first checklist line should not start with a backtick"
        )
        assert not body.endswith("`"), (
            f"Phase {phase} first checklist line should not end with a backtick"
        )
