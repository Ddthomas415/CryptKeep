...
def test_run_preflight_reports_auto_switched_ui_port(monkeypatch) -> None:
    ...
    monkeypatch.setattr(pw, "_port_can_bind", lambda host, port: int(port) == 8502)
    ...
    assert out["port_8501"]["requested_port"] == 8501
    assert out["port_8501"]["resolved_port"] == 8502
    assert out["port_8501"]["auto_switched"] is True
