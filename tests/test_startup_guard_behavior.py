...
from services.execution.startup_guard import StartupGuardError, require_known_flat_or_override
...
def test_unknown_state_halts(monkeypatch):
    monkeypatch.setattr("services.execution.startup_guard.PositionStateSQLite", lambda: DummyStore(None))
...
def test_open_position_halts_without_override(monkeypatch):
    monkeypatch.setattr("services.execution.startup_guard.PositionStateSQLite", lambda: DummyStore({"qty": 1.0}))
