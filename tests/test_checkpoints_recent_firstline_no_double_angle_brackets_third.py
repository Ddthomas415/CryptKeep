...
def test_recent_first_lines_do_not_contain_double_angle_brackets_third() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "<<" not in body, (
            f"Phase {phase} first checklist line should not contain double opening angle brackets"
        )
        assert ">>" not in body, (
            f"Phase {phase} first checklist line should not contain double closing angle brackets"
        )
