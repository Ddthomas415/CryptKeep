...
def test_recent_second_lines_have_no_backticked_test_tokens() -> None:
    for phase, body in _recent_second_line_bodies():
        test_tokens = [
            token for token in BACKTICK_RE.findall(body) if token.startswith("tests/")
        ]
        assert not test_tokens, (
            f"Phase {phase} second checklist line should not include backticked tests/ tokens"
        )
