...
def test_operator_read_only_command_status_reports_current_repo_wiring() -> None:
    ...
    assert rows["hetzner_paper_host_health"]["script"] == "scripts/report_hetzner_paper_host_health.py"
    assert rows["hetzner_paper_host_health"]["make_target"] == "check-hetzner-paper-host-health"
    assert rows["hetzner_paper_host_health"]["medium_lane_item"] == "host_status_wrapper"
    assert rows["hetzner_paper_host_health"]["input_class"] == "ssh_read_only"
