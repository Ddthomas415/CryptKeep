...
def test_live_enabled_and_armed_rejects_legacy_operator_arm_env(monkeypatch):
    monkeypatch.setattr(
        live_arming,
        "load_user_yaml",
        lambda: {"execution": {"live_enabled": True}},
...
def test_live_enabled_and_armed_accepts_boundary_compat_envs(monkeypatch):
    monkeypatch.setattr(
        live_arming,
        "load_user_yaml",
        lambda: {"execution": {"live_enabled": True}},
    ...
    monkeypatch.setenv("CBP_EXECUTION_LIVE_ENABLED", "1")
    ok, reason = live_arming.live_enabled_and_armed()
    assert ok is True
    assert reason == "env:CBP_EXECUTION_LIVE_ENABLED"
