...
ACTION_VERB_RE = re.compile(r"\b(added|hardened|fixed|cleaned)\b", re.IGNORECASE)
...
def test_recent_first_lines_have_single_action_verb_token() -> None:
    for phase, body in _recent_first_line_bodies():
        matches = ACTION_VERB_RE.findall(body)
        assert len(matches) == 1, (
            f"Phase {phase} first checklist line should include exactly one action-verb token"
        )
