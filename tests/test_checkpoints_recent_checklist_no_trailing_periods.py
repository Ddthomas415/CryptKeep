...
def _recent_checklist_bodies() -> list[tuple[int, str, str]]:
    ...
    for phase in verification_order[-RECENT_WINDOW_SIZE:]:
        ...
        rows.append((phase, bodies[-2], bodies[-1]))

    return rows


def test_recent_checklist_lines_have_no_trailing_periods() -> None:
    for phase, first, second in _recent_checklist_bodies():
        assert not first.rstrip().endswith('.'), (
            f"Phase {phase} first checklist line should not end with a period"
        )
        assert not second.rstrip().endswith('.'), (
            f"Phase {phase} second checklist line should not end with a period"
        )
