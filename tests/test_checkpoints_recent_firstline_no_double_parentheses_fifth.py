...
def test_recent_first_lines_do_not_contain_double_parentheses_fifth() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "((" not in body, (
            f"Phase {phase} first checklist line should not have doubled opening parentheses"
        )
        assert "))" not in body, (
            f"Phase {phase} first checklist line should not have doubled closing parentheses"
        )
