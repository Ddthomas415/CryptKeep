...
def test_recent_second_lines_have_no_commas() -> None:
    for phase, body in _recent_second_line_bodies():
        narrative = BACKTICK_RE.sub("", body)
        assert "," not in narrative, (
            f"Phase {phase} second checklist line should avoid comma-delimited multi-assertions"
        )
