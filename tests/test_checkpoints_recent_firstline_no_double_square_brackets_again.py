...
def test_recent_first_lines_do_not_contain_double_square_brackets_again() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "[[" not in body, (
            f"Phase {phase} first checklist line should not contain double opening square brackets"
        )
        assert "]]" not in body, (
            f"Phase {phase} first checklist line should not contain double closing square brackets"
        )
