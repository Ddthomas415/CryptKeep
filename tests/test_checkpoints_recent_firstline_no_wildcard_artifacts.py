...
def test_recent_firstline_artifacts_have_no_wildcards() -> None:
    for phase, artifact in _recent_firstline_artifacts():
        assert not any(ch in artifact for ch in "*?[]"), (
            f"Phase {phase} artifact should be concrete (no wildcard chars): {artifact}"
        )



def test_recent_firstline_artifacts_exist() -> None:
    for phase, artifact in _recent_firstline_artifacts():
        assert pathlib.Path(artifact).exists(), (
            f"Phase {phase} artifact path does not exist: {artifact}"
        )
