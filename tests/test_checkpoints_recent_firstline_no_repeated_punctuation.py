...
def test_recent_first_lines_do_not_contain_repeated_punctuation() -> None:
    for phase, body in _recent_first_line_bodies():
        assert not REPEATED_PUNCT_RE.search(body), (
            f"Phase {phase} first checklist line should not contain repeated punctuation tokens"
        )
