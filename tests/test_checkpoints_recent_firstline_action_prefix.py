from __future__ import annotations

import pathlib
import re


CHECKPOINTS_PATH = pathlib.Path("CHECKPOINTS.md")
PHASE_ENTRY_RE = re.compile(r"^- ✅ Phase (\d+):\s+(.*)$")
PHASE_VERIFICATION_RE = re.compile(r"^- ✅ Phase (\d+) verification:")
RECENT_WINDOW_SIZE = 12
ACTION_PREFIX_RE = re.compile(r"^(added|hardened|fixed|cleaned)\b", re.IGNORECASE)


def _recent_first_lines() -> list[tuple[int, str]]:
    lines = CHECKPOINTS_PATH.read_text(encoding="utf-8").splitlines()

    first_lines_by_phase: dict[int, str] = {}
    entry_counts: dict[int, int] = {}
    verification_order: list[int] = []

    for line in lines:
        if m := PHASE_ENTRY_RE.match(line):
            phase = int(m.group(1))
            body = m.group(2)
            entry_counts[phase] = entry_counts.get(phase, 0) + 1
            if entry_counts[phase] == 1:
                first_lines_by_phase[phase] = body
            continue
        if m := PHASE_VERIFICATION_RE.match(line):
            verification_order.append(int(m.group(1)))

    assert verification_order, "No phase verification lines found in CHECKPOINTS.md"

    out: list[tuple[int, str]] = []
    for phase in verification_order[-RECENT_WINDOW_SIZE:]:
        assert phase in first_lines_by_phase, f"Phase {phase} first checklist line not found"
        out.append((phase, first_lines_by_phase[phase]))
    return out


def test_recent_first_lines_start_with_action_prefix() -> None:
    for phase, line_body in _recent_first_lines():
        assert ACTION_PREFIX_RE.match(line_body), (
            f"Phase {phase} first checklist line should start with an action verb"
        )
