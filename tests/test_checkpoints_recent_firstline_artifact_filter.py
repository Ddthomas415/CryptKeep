...
def test_recent_first_lines_have_exactly_one_checkpoint_artifact_token() -> None:
    for phase, body in _recent_first_line_bodies():
        ...
        assert len(artifact_tokens) == 1, (
            f"Phase {phase} first checklist line should include exactly one checkpoint artifact token"
        )
