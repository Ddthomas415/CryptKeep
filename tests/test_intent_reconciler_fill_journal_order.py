...
def test_reconciler_journals_fills_before_marking_filled(monkeypatch):
    ...
    def fake_paper_queue_status(qdb, intent, status, *, ctx, last_error=None, client_order_id=None, linked_order_id=None):
        calls.append(("update_status", intent["intent_id"], status, linked_order_id))
        return True
    ...
    monkeypatch.setattr(mod, "paper_queue_status", fake_paper_queue_status)
