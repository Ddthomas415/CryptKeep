...
def test_recent_first_lines_contain_backticked_artifact_reference() -> None:
    for phase, body in _recent_first_line_bodies():
        assert BACKTICK_RE.search(body), (
            f"Phase {phase} first checklist line should include a backticked artifact token"
        )
