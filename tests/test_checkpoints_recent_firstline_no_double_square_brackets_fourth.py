...
def test_recent_first_lines_do_not_contain_double_square_brackets_fourth() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "[[" not in body, (
            f"Phase {phase} first checklist line should not contain doubled opening square brackets"
        )
        assert "]]" not in body, (
            f"Phase {phase} first checklist line should not contain doubled closing square brackets"
        )
