...
def _recent_first_line_bodies() -> list[tuple[int, str]]:
    ...
    entry_bodies_by_phase: dict[int, list[str]] = {}
    verification_order: list[int] = []

    for line in lines:
        if m := PHASE_ENTRY_RE.match(line):
            phase = int(m.group(1))
            entry_bodies_by_phase.setdefault(phase, []).append(m.group(2))
            continue
    ...
    for phase in verification_order[-RECENT_WINDOW_SIZE:]:
        bodies = entry_bodies_by_phase.get(phase, [])
        assert len(bodies) >= 2, f"Phase {phase} must have at least two checklist entries"
        out.append((phase, bodies[-2]))
    return out


def test_recent_first_lines_have_no_commas() -> None:
    for phase, body in _recent_first_line_bodies():
        assert "," not in body, (
            f"Phase {phase} first checklist line should avoid comma-delimited multi-assertions"
        )
