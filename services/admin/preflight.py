...
def _safe_private_connectivity(exchange: str) -> dict:
    try:
        return test_private_connectivity(exchange)
    except Exception as e:
        return {"ok": False, "exchange": str(exchange), "reason": type(e).__name__, "error": str(e)[:600]}


def _safe_probes(exchange: str, probe_keys: list[str]) -> dict:
    try:
        return run_probes(exchange, probe_keys)
    except Exception as e:
        return {
            "ok": False,
...
async def run_preflight(
    ...
    *,
    ...
    for venue in venue_list:
        conn = _safe_private_connectivity(venue)
        probe = _safe_probes(venue, probes)
