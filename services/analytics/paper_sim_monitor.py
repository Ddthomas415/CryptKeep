...
def _paper_state_snapshot(symbol: str) -> dict[str, Any]:
    return _paper_state_snapshot_window(symbol, since_ts="", until_ts="")
...
def _row_meets_window(row: dict[str, Any], *, since_epoch: float, until_epoch: float, keys: list[str]) -> bool:
    row_epoch = 0.0
    for key in keys:
        row_epoch = _iso_epoch(row.get(key))
        if row_epoch > 0.0:
            break
    if row_epoch <= 0.0:
        return since_epoch <= 0.0 and until_epoch <= 0.0
    if since_epoch > 0.0 and row_epoch < since_epoch:
        return False
    if until_epoch > 0.0 and row_epoch > until_epoch:
        return False
    return True


def _paper_state_snapshot_window(symbol: str, *, since_ts: str = "", until_ts: str = "") -> dict[str, Any]:
    ...
    until_epoch = _iso_epoch(until_ts)
    latest_order_rows = [
        dict(row)
        for row in list(store.list_orders(limit=500) or [])
        if not symbol or str(row.get("symbol") or "").strip() == str(symbol).strip()
        if _row_meets_window(row, since_epoch=since_epoch, until_epoch=until_epoch, keys=["created_ts", "ts"])
    ]
    ...
    for row in latest_order_rows:
        ...
        matching_fills = [
            ...
            if _row_meets_window(dict(fill), since_epoch=since_epoch, until_epoch=until_epoch, keys=["ts"])
        ]
    ...
    if until_epoch > 0.0 and not latest_order and not latest_paper_fill:
        position = {}
    return {
...
def _trade_journal_snapshot(symbol: str, *, since_ts: str = "", until_ts: str = "") -> dict[str, Any]:
    ...
    for row in rows:
        if (
            ...
            and _row_meets_window(dict(row), since_epoch=since_epoch, until_epoch=until_epoch, keys=["fill_ts", "journal_ts"])
        ):
            return dict(row)
    return {}
...
def collect_once(cfg: PaperSimMonitorCfg) -> dict[str, Any]:
    ...
    window_ended_ts = str(
        result.get("ended_ts")
        or (campaign.get("ts") if campaign_status in {"completed", "failed", "dead", "stopped"} else "")
        or ""
    ).strip()
    paper_state = _paper_state_snapshot_window(symbol, since_ts=window_started_ts, until_ts=window_ended_ts)
    ...
    latest_journal_fill = _trade_journal_snapshot(symbol, since_ts=window_started_ts, until_ts=window_ended_ts)
    strategy_name = str(
...
def run_forever(cfg: PaperSimMonitorCfg, *, max_loops: int | None = None) -> dict[str, Any]:
    ...
    while True:
        if stop_file().exists():
            ...
            out["reason"] = _terminal_monitor_reason(last_snapshot)
            _write_status(out)
            _clear_pid_state()
            return out
