...
from services.analytics.funding_stage0_readiness import (
    CONTEXT_SOURCE,
    CONTEXT_SYMBOL,
    CONTEXT_VENUE,
    SESSION_STRATEGY_ID,
...
BASELINE_REPORT_TYPE = "funding_stage0_baseline"
VERIFICATION_REPORT_TYPE = "funding_stage0_verification"
...
def _journal_count(path: Path, *, strategy_id: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        return {"exists": True, "count": 0, "path": str(path), "error": f"{type(exc).__name__}:{exc}"}
...
def _latest_completed_after(rows: list[dict[str, Any]], *, baseline_ts: datetime | None) -> dict[str, Any]:
...
def build_funding_stage0_baseline(*, repo_root: Path | None = None, expected_commit: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "operator_next_step": "Run the accepted 15-minute Stage 0 proof command, then run verification.",
    }


def _load_baseline(path: Path | None) -> tuple[dict[str, Any], str]:
    baseline_path = path or (data_dir() / "funding_stage0_verification" / "funding_stage0_baseline.latest.json")
...
def build_funding_stage0_verification(
    *,
    ...
    checks = [
        ...
        _check("completed_session_after_baseline", bool(completed), str(completed.get("timestamp") or "missing")),
        _check(
        ...
        _check("completed_session_reconciled", str(completed.get("reconciliation_result") or "") == "pass", str(completed.get("reconciliation_result") or "")),
        _check("completed_session_no_critical_error", bool(completed.get("critical_error")) is False, str(completed.get("critical_error"))),
        _check("completed_session_public_ohlcv", provenance_ok, json.dumps(completed, sort_keys=True)),
        _check(
        ...
        _check(
            ...
            bool(strategy_status.get("signal_ok")) is True
            and str(strategy_status.get("signal_reason") or "") not in {"", "no_public_ohlcv", "missing_funding_context"},
            str(strategy_status.get("signal_reason") or ""),
        ),
        _check(
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Funding Extreme Stage 0 Proof Report",
            "",
...
def _artifact_dir() -> Path:
    return data_dir() / "funding_stage0_verification"


def write_funding_stage0_baseline(report: dict[str, Any]) -> dict[str, str]:
    out_dir = _artifact_dir()
    latest_json = out_dir / "funding_stage0_baseline.latest.json"
    latest_markdown = out_dir / "funding_stage0_baseline.latest.md"
    stamped_json = out_dir / f"funding_stage0_baseline.{_safe_stamp()}.json"
    stamped_markdown = out_dir / f"funding_stage0_baseline.{_safe_stamp()}.md"
    return _write_report(report, latest_json, latest_markdown, stamped_json, stamped_markdown)


def write_funding_stage0_verification(report: dict[str, Any]) -> dict[str, str]:
    out_dir = _artifact_dir()
    latest_json = out_dir / "funding_stage0_verification.latest.json"
    latest_markdown = out_dir / "funding_stage0_verification.latest.md"
    stamped_json = out_dir / f"funding_stage0_verification.{_safe_stamp()}.json"
    stamped_markdown = out_dir / f"funding_stage0_verification.{_safe_stamp()}.md"
    return _write_report(report, latest_json, latest_markdown, stamped_json, stamped_markdown)
