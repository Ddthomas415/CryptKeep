...
CREATE UNIQUE INDEX IF NOT EXISTS idx_fills_fill_id
  ON fills(venue, fill_id) WHERE fill_id IS NOT NULL;
...
class PnLStoreSQLite:
    ...
    def _connect(self) -> sqlite3.Connection:
        conn = sqlite3.connect(self.db, isolation_level="DEFERRED", check_same_thread=False)
    ...
    async def record_fill(self, venue: str, symbol: str, side: str, qty: float, price: float,
                          fee: float = 0.0, fee_ccy: str | None = None,
                          fill_id: str | None = None) -> Dict[str, Any]:
        return await asyncio.to_thread(self._record_fill_sync, venue, symbol, side, qty, price, fee, fee_ccy, fill_id)

    def _record_fill_sync(self, venue: str, symbol: str, side: str, qty: float,
                          price: float, fee: float = 0.0, fee_ccy: str | None = None,
                          fill_id: str | None = None) -> Dict[str, Any]:
        venue = str(venue)
        symbol = str(symbol)
        side = str(side).lower()
        ...
        try:
            with conn:
                cur = conn.execute(
                    "INSERT OR IGNORE INTO fills(ts, venue, symbol, side, qty, price, fee, fee_ccy, fill_id) VALUES(?,?,?,?,?,?,?,?,?)",
                    (_now(), venue, symbol, side, q, px, fee, (str(fee_ccy) if fee_ccy else None), (str(fill_id) if fill_id else None)),
                )
                if fill_id and cur.rowcount == 0:
                    return {"ok": True, "realized_delta": 0.0, "duplicate_fill": True}
                row = conn.execute(
