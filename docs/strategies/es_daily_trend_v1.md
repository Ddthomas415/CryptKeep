...
`net_return_pct`, so average winning and losing trades are compared as net
return after allocated fees divided by entry notional. If `win_rate`,
`avg_win_return_pct`, or `avg_loss_return_pct` is unset, the gate must keep
`manual_review_required=true` rather than assuming the comparison passed.
