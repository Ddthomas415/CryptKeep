# Strategy Decision Record — 2026-05-15

## Scope

This record reflects the current repo state on 2026-05-15.
...
- previous run: `2026-04-08T19:11:49Z`
- current run: `2026-05-15T02:02:46Z`
- top strategy previous: `breakout_donchian`
- top strategy current: `breakout_donchian`
- top strategy changed: `no`
- improved comparisons: `3`
- degraded comparisons: `0`
- unchanged comparisons: `0`
- new comparisons: `5`

Summary: 3 strategy comparison(s) improved versus the prior persisted evidence run.
...
Comparison detail:
- `breakout_donchian` moved `improved`; rank `1` -> `1`, decision `improve` -> `improve`.
- `ema_cross` moved `improved`; rank `2` -> `2`, decision `improve` -> `improve`.
- `mean_reversion_rsi` moved `improved`; rank `3` -> `3`, decision `freeze` -> `freeze`.
- `momentum` moved `new`; rank `-` -> `4`, decision `-` -> `retire`.
- `volatility_reversal` moved `new`; rank `-` -> `5`, decision `-` -> `freeze`.
...
- evidence status: `insufficient`
- confidence: `low`
- paper-history: 6 closed trade(s), -1.58 net realized PnL, 16.7% win rate across 12 fill(s).
- strategy feedback: 6 closed trade(s), -1.58 net realized PnL, -0.26 expectancy per closed trade, 16.7% win rate.
...
- Evidence note: No realized closed-trade participation exists across the current evidence windows.
- Feedback weighting: Persisted paper feedback is negative or fragile for this strategy (-0.26 expectancy, -1.58 net realized PnL), so the research leaderboard applies a conservative penalty.
- Biggest weakness: No realized trading participation across the current evidence windows.
