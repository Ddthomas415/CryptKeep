...
st.divider()
st.header("Daily Risk Rollup (Phase 83)")

st.caption("Deterministic daily counters used by LIVE gates: trades_today + realized_pnl_usd_today.")
...
try:
    ...
    st.json(RiskDailyDB(exec_db).get())
except Exception as e:
    st.error(f"Phase 83 panel error: {type(e).__name__}: {e}")

st.divider()
st.header("Fill Ledger Quick Test (Phase 84)")

st.caption("Writes a dummy fill into fills_ledger and updates risk_daily. Use this to confirm LIVE gates have a PnL source.")

try:
    import subprocess, sys
    sym = st.text_input("Dummy symbol", value="BTC-USD", key="p84_sym")
    pnl = st.number_input("Dummy realized PnL USD", value=0.0, step=1.0, key="p84_pnl")
    fee = st.number_input("Dummy fee USD", value=0.0, step=0.1, key="p84_fee")
    if st.button("Record dummy fill", key="p84_btn"):
        out = subprocess.check_output([sys.executable, "scripts/record_dummy_fill.py", "--symbol", sym, "--pnl", str(pnl), "--fee", str(fee)], stderr=subprocess.STDOUT, text=True)
        st.code(out)
except Exception as e:
    st.error(f"{type(e).__name__}: {e}")

st.divider()
st.header("LIVE Fills Poller (Phase 88)")
st.caption("REST-polls exchanges via CCXT fetch_my_trades and writes fills to canonical tables. SAFE: no orders placed.")

try:
    import yaml
    from services.fills.fills_poller import load_cfg
    from services.risk.live_risk_gates import LiveGateDB

    cfg = load_cfg("config/trading.yaml")
    db = LiveGateDB(exec_db=cfg.exec_db)

    for ex in cfg.exchanges:
        hb = db.get_state(f"fills_poller_last::{ex}", "")
        since = db.get_state(f"fills_since_ms::{ex}", "")
        err = db.get_state(f"fills_poller_err::{ex}", "")
        st.markdown(f"**{ex.upper()}** - last heartbeat: {hb}, cursor: {since}, last error: {err}")
except Exception as e:
    st.caption(f"Fills Poller panel error: {type(e).__name__}: {e}")

st.markdown("### FillSink quick test (Phase 87)")
st.caption("Writes a synthetic fill to exec_db canonical tables (no orders).")
try:
    import subprocess, sys
    if st.button("Inject synthetic test fill"):
        try:
            out = subprocess.check_output([sys.executable, "scripts/inject_test_fill.py"], stderr=subprocess.STDOUT, text=True)
            st.code(out)
        except subprocess.CalledProcessError as e:
            st.code(e.output or str(e))
except Exception as e:
    st.caption(f"FillSink test error: {type(e).__name__}: {e}")

st.divider()
st.header("Market Rules (Phase 87)")

st.caption("Refresh per-venue market rules cache, then validate symbol + min order constraints. LIVE should fail-closed if rules are missing.")

import subprocess, sys

venue = st.selectbox("Venue", ["binance","coinbase","gate"], index=0, key="mr_venue")
default_sym = "BTC-USDT" if venue != "coinbase" else "BTC-USD"
symbol = st.text_input("Canonical symbol", value=default_sym, key="mr_sym")
qty = st.number_input("Test qty (optional)", value=0.0, min_value=0.0, step=0.001, key="mr_qty")
notional = st.number_input("Test notional USD (optional)", value=0.0, min_value=0.0, step=10.0, key="mr_notional")

col1, col2 = st.columns(2)
with col1:
    if st.button("Refresh rules cache", use_container_width=True, key="mr_refresh"):
        try:
            out = subprocess.check_output([sys.executable, "scripts/refresh_market_rules.py", "--venue", venue, "--symbols", symbol], stderr=subprocess.STDOUT, text=True)
            st.code(out)
        except Exception as e:
            st.error(str(e))

with col2:
    if st.button("Validate now", use_container_width=True, key="mr_validate"):
        try:
            import yaml
            cfg = yaml.safe_load(open("config/trading.yaml","r",encoding="utf-8").read()) or {}
            ex_cfg = cfg.get("execution") or {}
            ...
            vr = validate(exec_db, venue, symbol, qty=(qty if qty>0 else None), notional=(notional if notional>0 else None))
            st.json({"ok": vr.ok, "code": vr.code, "message": vr.message, "rules": None if not vr.rules else vr.rules.__dict__})
        except Exception as e:
            st.error(f"{type(e).__name__}: {e}")

st.divider()
st.header("Market Rules Cache Health (Phase 88)")

st.caption("LIVE prereqs should fail-closed unless the market rules cache is fresh. Refresh via scripts/refresh_market_rules.py.")

try:
    from services.markets.prereq import check_market_rules_prereq
    pr = check_market_rules_prereq(ttl_s=6*3600.0)
    if pr.ok:
