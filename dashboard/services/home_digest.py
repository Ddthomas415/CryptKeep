...
def _utc_iso(dt: datetime | None = None) -> str:
    stamp = (dt or _utc_now()).astimezone(timezone.utc).replace(microsecond=0)
...
def _fmt_pct_abs(value: Any) -> str:
    return f"{_coerce_float(value, 0.0):.2f}%"
...
def _freshness_state_from_age(age_seconds: int | None) -> FreshnessState:
...
def _freshness_state_from_label(label: Any, *, age_seconds: int | None = None) -> FreshnessState:
    ...
    if value in {"no live data", "missing", "unknown", "unavailable", "", "no snapshots"}:
        return "missing" if age_seconds is None else _freshness_state_from_age(age_seconds)
    return _freshness_state_from_age(age_seconds)


def _health_from_freshness(state: FreshnessState) -> HealthState:
    if state == "fresh":
        return "ok"
    if state == "aging":
        return "warn"
...
def _normalize_health_state(value: Any, *, default: HealthState = "unknown") -> HealthState:
    ...
    *,
    ...
    return RuntimeTruthData(
        **_base_section(
            ...
            source_name="config/trading.yaml + live_guard + collector_runtime",
    ...
    *,
    ...
    return AttentionNowData(
        **_base_section(
            ...
            source_name="overview summary + operations snapshot + structural freshness",
...
def build_leaderboard_summary_digest(*, as_of: str, strategy_context: dict[str, Any]) -> LeaderboardSummaryData:
    ...
    return LeaderboardSummaryData(
        **_base_section(
            ...
            source_name="services.backtest.leaderboard.run_strategy_leaderboard",
            source_age_seconds=0,
        ),
        rows=rows,
    )
...
def build_scorecard_snapshot_digest(*, as_of: str, strategy_context: dict[str, Any]) -> ScorecardSnapshotData:
    ...
    return ScorecardSnapshotData(
        **_base_section(
            ...
            source_name="services.backtest.scorecard via run_strategy_leaderboard",
            source_age_seconds=0,
        ),
...
def build_crypto_edge_summary_digest(*, as_of: str, live_snapshot: dict[str, Any]) -> CryptoEdgeSummaryData:
    ...
    return CryptoEdgeSummaryData(
        **_base_section(
            ...
            source_name="dashboard.services.crypto_edge_research.load_latest_live_crypto_edge_snapshot",
    ...
    *,
    ...
    return SafetyWarningsData(
        **_base_section(
            ...
            source_name="live_guard + start_manager + collector_runtime",
            source_age_seconds=0,
        ),
        items=items[:6],
    ...
    *,
    ...
    return FreshnessPanelData(
        **_base_section(
            ...
            source_name="collector runtime + latest live snapshot + synthetic evaluation",
            source_age_seconds=0,
        ),
        rows=rows,
    )
...
def build_mode_truth_digest(*, as_of: str, runtime_context: dict[str, Any]) -> ModeTruthData:
    ...
    return ModeTruthData(
        **_base_section(
            ...
            source_name="config/trading.yaml + services.bot.start_manager.decide_start",
            source_age_seconds=0,
        ),
    ...
    *,
    ...
    return RecentIncidentsData(
        **_base_section(
            ...
            source_name="overview warnings + collector runtime + operations snapshot",
            source_age_seconds=0,
        ),
        items=items[:6],
    )
    ...
    if top_row:
        return NextBestActionData(
            **_base_section(
                ...
                source_name="strategy_leaderboard",
                source_age_seconds=0,
            ),
