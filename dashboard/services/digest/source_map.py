...
DIGEST_SOURCE_MAP = {
    "runtime_truth": "merged runtime trading config + live_guard + system_guard + collector_runtime",
    ...
    "safety_warnings": "live_guard + system_guard + start decision + collector_runtime",
    "freshness_panel": "collector runtime + latest live snapshot + strategy evidence artifact or labeled synthetic fallback",
    "mode_truth": "merged runtime trading config + services.bot.start_manager.decide_start (decision-only) + dashboard.services.promotion_ladder.build_promotion_readiness",
