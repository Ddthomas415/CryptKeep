...
def test_terminal_execute_status_allowed() -> None:
    response = analyst_client.post("/api/v1/terminal/execute", json={"command": "status"})
...
def test_terminal_execute_kill_switch_requires_confirmation() -> None:
    response = owner_client.post("/api/v1/terminal/execute", json={"command": "kill-switch on"})
    assert response.status_code == 200
...
def test_terminal_confirm_token_is_single_use() -> None:
    execute_response = owner_client.post("/api/v1/terminal/execute", json={"command": "kill-switch on"})
    assert execute_response.status_code == 200
    execute_payload = execute_response.json()
    token = execute_payload["data"]["confirmation_token"]

    confirm_response = owner_client.post("/api/v1/terminal/confirm", json={"confirmation_token": token})
    assert confirm_response.status_code == 200
    confirm_payload = confirm_response.json()
    assert confirm_payload["status"] == "success"
    assert confirm_payload["data"]["confirmed"] is True

    confirm_again_response = owner_client.post("/api/v1/terminal/confirm", json={"confirmation_token": token})
...
def test_terminal_confirmation_tokens_are_unique() -> None:
    first_execute_response = owner_client.post("/api/v1/terminal/execute", json={"command": "kill-switch on"})
    assert first_execute_response.status_code == 200
    first_token = first_execute_response.json()["data"]["confirmation_token"]

    second_execute_response = owner_client.post("/api/v1/terminal/execute", json={"command": "mode set paper"})
...
def test_terminal_execute_rejects_non_approved_command() -> None:
    response = analyst_client.post("/api/v1/terminal/execute", json={"command": "rm -rf /"})
    assert response.status_code == 200
...
def test_terminal_execute_rejects_shell_chaining_after_approved_prefix() -> None:
    response = analyst_client.post("/api/v1/terminal/execute", json={"command": "logs tail; rm -rf /"})
    assert response.status_code == 200
...
def test_terminal_execute_blocks_dangerous_command_for_analyst() -> None:
    response = analyst_client.post("/api/v1/terminal/execute", json={"command": "kill-switch on"})
    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "success"
    assert payload["data"]["requires_confirmation"] is False
    assert payload["data"]["output"][0]["type"] == "error"
    assert "role cannot run dangerous" in payload["data"]["output"][0]["value"].lower()


def test_terminal_confirmation_token_is_bound_to_subject() -> None:
    execute_response = owner_client.post("/api/v1/terminal/execute", json={"command": "kill-switch on"})
    token = execute_response.json()["data"]["confirmation_token"]
    confirm_response = analyst_client.post("/api/v1/terminal/confirm", json={"confirmation_token": token})
    assert confirm_response.status_code == 400
