...
@router.get("/summary", response_model=ApiEnvelope[RiskSummary])
def risk_summary(request: Request) -> dict:
    data = service.get_summary()
    return success(data=data, request_id=request.state.request_id)


@router.get("/limits", response_model=ApiEnvelope[RiskLimits])
def risk_limits(request: Request) -> dict:
    data = service.get_limits()
    return success(data=data, request_id=request.state.request_id)


@router.put("/limits", response_model=ApiEnvelope[RiskLimits])
