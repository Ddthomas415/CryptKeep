...
HETZNER_MULTI_VENUE_PROPOSAL_ARGS ?=
...
HETZNER_GATEIO_CHALLENGER_CONFIG ?= configs/paper_evidence_campaigns.hetzner.gateio_challenger.json
HETZNER_GATEIO_CHALLENGER_NAME ?= ema_cross_gateio_btcusdt_paper_candidate
HETZNER_BINANCE_CHALLENGER_CONFIG ?= configs/paper_evidence_campaigns.hetzner.binance_challenger.json
HETZNER_BINANCE_CHALLENGER_NAME ?= ema_cross_binance_btcusdt_paper_candidate
HETZNER_STATUS_TIMEOUT_SEC ?= 15
HETZNER_STATUS_TRANSPORT ?= ssh
...
HETZNER_DEPENDENCY_EXPECTED_COMMIT ?= $(shell git rev-parse HEAD)
HETZNER_DEPENDENCY_EXPECTED_BRANCH ?= master
...
.PHONY: doctor-strict alignment check-alignment check-alignment-list check-alignment-list-json check-alignment-json check-alignment-json-fast validate-quick validate-json-quick validate-json-fast validate-json validate pre-release-sanity pre-release-sanity-quick pre-release-sanity-json-quick pre-release-sanity-json-fast remaining-tasks roadmap-tracking-status roadmap-tracking-status-json backlog-lane-status backlog-lane-status-json operator-proof-status operator-proof-status-json operator-read-only-command-status operator-read-only-command-status-json operator-status operator-status-json operator-next-actions operator-next-actions-json operator-next-actions-passive operator-next-actions-passive-json operator-briefing operator-briefing-json record-operator-briefing operator-event-secrets operator-event-secrets-json record-operator-event-secrets record-ai-provider-event-secrets record-ai-report-event-secrets credential-source-posture credential-source-posture-json platform-event-journal platform-event-journal-json platform-event-secrets platform-event-secrets-json platform-event-integrity platform-event-integrity-json platform-event-packet platform-event-packet-json operator-arm-to-halt-replay operator-arm-to-halt-replay-json record-operator-arm-to-halt-replay record-manual-strategy-performance-decision record-composite-hybrid-paper-decision record-funding-extreme-persistent-campaign-decision record-hetzner-state-migration-checkpoint record-paper-to-shadow-first-hour-checkpoint record-backup-restore-drill-checkpoint record-server-secrets-rotation-checkpoint phase1-safety phase1-smoke phase1-smoke-openai smoke-exchange-sandbox record-exchange-sandbox-smoke load-sample-crypto-edges collect-live-crypto-edges collect-live-crypto-edges-loop stop-live-crypto-edges-loop status-live-crypto-edges-loop check-short-context-readiness check-edge-cadence check-edge-cadence-json check-dead-man check-dead-man-json check-cost-assumptions check-cost-assumptions-json record-cost-assumptions check-supply-chain check-supply-chain-json record-supply-chain record-execution-cost-stack backup-state check-backup-artifact-secrets collect-paper-strategy-evidence stop-paper-strategy-evidence status-paper-strategy-evidence status-paper-campaigns status-paper-soak status-paper-soak-json status-paper-gate-qualification status-paper-gate-qualification-json plan-multi-symbol-paper-campaigns plan-multi-symbol-paper-campaigns-json status-hetzner-multi-venue-proposals status-hetzner-multi-venue-proposals-json status-paper-hetzner status-hetzner-gateio-challenger restore-hetzner-gateio-challenger status-hetzner-binance-challenger restore-hetzner-binance-challenger status-hetzner-edge-runtime status-hetzner-dependency-alignment status-hetzner-dependency-alignment-json status-paper-all check-hetzner-paper-host-health check-hetzner-paper-host-health-local restore-paper-campaigns recover-paper-campaigns strategy-evidence-cycle system-diagnostics dashboard docker-up-auto-ports docker-print-auto-ports test test-runtime test-checkpoints ai-operator-oversight research-pipeline-status research-pipeline-status-json research-command-status research-command-status-json
...
record-operator-briefing:
	$(PYTHON) scripts/report_operator_briefing.py --write-default-artifact --config $(PAPER_CAMPAIGN_CONFIG) --max-actions $(OPERATOR_BRIEFING_MAX_ACTIONS)
...
status-hetzner-multi-venue-proposals:
	$(PYTHON) scripts/report_hetzner_multi_venue_proposal_status.py $(HETZNER_MULTI_VENUE_PROPOSAL_ARGS)

status-hetzner-multi-venue-proposals-json:
	@$(PYTHON) scripts/report_hetzner_multi_venue_proposal_status.py --json $(HETZNER_MULTI_VENUE_PROPOSAL_ARGS)
...
status-hetzner-gateio-challenger:
	$(MAKE) --no-print-directory status-paper-hetzner HETZNER_PAPER_CAMPAIGN_CONFIG=$(HETZNER_GATEIO_CHALLENGER_CONFIG)

restore-hetzner-gateio-challenger:
	ssh -o BatchMode=yes $(HETZNER_SSH_TARGET) 'cd $(HETZNER_APP_DIR) && ./.venv/bin/python scripts/restore_paper_campaigns.py --config $(HETZNER_GATEIO_CHALLENGER_CONFIG) --campaign $(HETZNER_GATEIO_CHALLENGER_NAME) --restore --preflight-ohlcv'

status-hetzner-binance-challenger:
	$(MAKE) --no-print-directory status-paper-hetzner HETZNER_PAPER_CAMPAIGN_CONFIG=$(HETZNER_BINANCE_CHALLENGER_CONFIG)

restore-hetzner-binance-challenger:
	ssh -o BatchMode=yes $(HETZNER_SSH_TARGET) 'cd $(HETZNER_APP_DIR) && CBP_VENUE=binance CBP_ALLOW_BINANCE=1 ./.venv/bin/python scripts/restore_paper_campaigns.py --config $(HETZNER_BINANCE_CHALLENGER_CONFIG) --campaign $(HETZNER_BINANCE_CHALLENGER_NAME) --restore --preflight-ohlcv'
...
status-hetzner-dependency-alignment:
	$(PYTHON) scripts/report_hetzner_dependency_alignment_status.py --strict --ssh-target $(HETZNER_SSH_TARGET) --transport $(HETZNER_STATUS_TRANSPORT) --app-dir $(HETZNER_APP_DIR) --expected-branch $(HETZNER_DEPENDENCY_EXPECTED_BRANCH) --expected-commit $(HETZNER_DEPENDENCY_EXPECTED_COMMIT) --timeout-sec 90

status-hetzner-dependency-alignment-json:
	@$(PYTHON) scripts/report_hetzner_dependency_alignment_status.py --json --ssh-target $(HETZNER_SSH_TARGET) --transport $(HETZNER_STATUS_TRANSPORT) --app-dir $(HETZNER_APP_DIR) --expected-branch $(HETZNER_DEPENDENCY_EXPECTED_BRANCH) --expected-commit $(HETZNER_DEPENDENCY_EXPECTED_COMMIT) --timeout-sec 90
...
check-hetzner-paper-host-health:
	$(PYTHON) scripts/report_hetzner_paper_host_health.py --strict --ssh-target $(HETZNER_SSH_TARGET) --transport $(HETZNER_STATUS_TRANSPORT) --app-dir $(HETZNER_APP_DIR) --config $(HETZNER_PAPER_CAMPAIGN_CONFIG) --timeout-sec $(HETZNER_STATUS_TIMEOUT_SEC)

check-hetzner-paper-host-health-local:
...
script-index:
	...
	@echo "  make operator-briefing[-json] — advisory read-only operator briefing from existing status reports"
