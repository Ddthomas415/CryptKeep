...
| `report_operator_next_actions.py` | `make operator-next-actions` / `make operator-next-actions-json` | Read-only compact next-action report derived from operator status; supports `OPERATOR_NEXT_ACTIONS_MAX`, `OPERATOR_NEXT_ACTIONS_LANE`, and `OPERATOR_NEXT_ACTIONS_REASON`; does not run research, campaigns, market-data fetches, proof closure, or runtime mutation |
