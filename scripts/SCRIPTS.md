...
| `report_operator_briefing.py` | `make operator-briefing` / `make operator-briefing-json` | Read-only advisory briefing over existing operator status, next-action, paper-gate velocity, and cost-assumption reports; summarizes campaign/gate/cost/action state and recommendations without moving capital, starting/stopping campaigns, fetching market data, changing config, promoting strategies, or mutating runtime state |
