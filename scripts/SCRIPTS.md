...
| `report_operator_briefing.py` | `make operator-briefing` / `make operator-briefing-json` | Read-only advisory briefing over existing operator status, next-action, paper-gate velocity, cost-assumption, and campaign-status reports; uses `PAPER_CAMPAIGN_CONFIG` for the campaign section so it matches `status-paper-campaigns`; summarizes campaign/gate/cost/action state and recommendations without moving capital, starting/stopping campaigns, fetching market data, changing config, promoting strategies, or mutating runtime state |
...
- `report_operator_briefing.py` — read-only advisory briefing over existing
  ...
  reports; use `make operator-briefing` or `make operator-briefing-json`.
