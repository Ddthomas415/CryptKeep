...
| `report_operator_proof_status.py` | `make operator-proof-status` / `make operator-proof-status-json` | Read-only report over passive/operator-evidence lane items and proof/coverage markers in `REMAINING_TASKS.md`; includes proof next actions without running campaigns, fetching market data, closing proof, or mutating state |
