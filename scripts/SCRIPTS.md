...
| `report_paper_gate_velocity.py` | `make status-paper-gate-velocity` / `make status-paper-gate-velocity-json` | Read-only paper-gate velocity report; estimates completion from completed provenance-qualified round-trip close cadence and qualified source-bar cadence, then reports the slower active blocker while keeping legacy/all-history fills diagnostic only |
