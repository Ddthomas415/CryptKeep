...
| `report_operator_next_actions.py` | `make operator-next-actions` / `make operator-next-actions-json` | Read-only compact next-action report derived from operator status; does not run research, campaigns, market-data fetches, proof closure, or runtime mutation |
