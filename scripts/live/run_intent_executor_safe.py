...
def _log_path() -> Path:
    path = app_paths.runtime_dir() / "logs" / "intent_executor.log"
...
def main() -> int:
    ...
    if not ok:
        ...
        try:
            ...
            log("intent_executor stopped (KeyboardInterrupt)")
            return 0

    log("intent_executor wrapper launching real module: scripts.run_intent_executor")

    try:
        # Execute the existing real script module as __main__.
        runpy.run_module("scripts.compat.run_intent_executor", run_name="__main__")
        return 0
    except KeyboardInterrupt:
        log("intent_executor stopped (KeyboardInterrupt)")
        return 0
    except Exception as e:
        log("intent_executor crashed: " + repr(e))
