...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write a research-only multi-window price-action stability report "
            "from archived OHLCV. It compares label-conditioned forward returns "
            "against unconditioned baselines without changing campaigns, gates, "
            "strategy config, or execution."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_price_action_window_stability(
    ...
    result["script"] = "scripts/research/run_price_action_window_stability.py"
