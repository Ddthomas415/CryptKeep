...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write a research-only price-action label forward-return report "
            "from archived OHLCV. The report is descriptive and does not "
            "modify strategy config, campaigns, gates, or execution."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_price_action_forward_returns(
    ...
    result["script"] = "scripts/research/run_price_action_forward_returns.py"
